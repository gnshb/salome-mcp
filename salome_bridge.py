...
class SalomeRuntime:
    ...
    def create_naca4_airfoil(
        ...
        tol = max(1e-9, abs(chord_value) * 1e-9)

        upper_points = upper
        lower_points = list(reversed(lower))

        # Avoid duplicate TE point when upper/lower coincide (closed trailing edge).
        if lower_points:
            te_gap = math.sqrt(
                (upper_points[-1][0] - lower_points[0][0]) ** 2
                + (upper_points[-1][1] - lower_points[0][1]) ** 2
                + (upper_points[-1][2] - lower_points[0][2]) ** 2
            )
            if te_gap <= tol:
                lower_points = lower_points[1:]

        upper_vertices = [geompy.MakeVertex(px, py, pz) for px, py, pz in upper_points]
        lower_vertices = [geompy.MakeVertex(px, py, pz) for px, py, pz in lower_points]

        edges: List[Any] = []
        if len(upper_vertices) >= 2:
            edges.append(geompy.MakeInterpol(upper_vertices))

        if upper_vertices and lower_vertices:
            te_gap = math.sqrt(
                (upper_points[-1][0] - lower_points[0][0]) ** 2
                + (upper_points[-1][1] - lower_points[0][1]) ** 2
                + (upper_points[-1][2] - lower_points[0][2]) ** 2
            ...
            if te_gap > tol:
                try:
                    edges.append(geompy.MakeLineTwoPnt(upper_vertices[-1], lower_vertices[0]))
                except Exception:
                    edges.append(geompy.MakeEdge(upper_vertices[-1], lower_vertices[0]))

        if len(lower_vertices) >= 2:
            edges.append(geompy.MakeInterpol(lower_vertices))

        if upper_vertices and lower_vertices:
            le_gap = math.sqrt(
                (lower_points[-1][0] - upper_points[0][0]) ** 2
                + (lower_points[-1][1] - upper_points[0][1]) ** 2
                + (lower_points[-1][2] - upper_points[0][2]) ** 2
            )
            if le_gap > tol:
                try:
                    edges.append(geompy.MakeLineTwoPnt(lower_vertices[-1], upper_vertices[0]))
                except Exception:
                    edges.append(geompy.MakeEdge(lower_vertices[-1], upper_vertices[0]))

        if not edges:
            raise RuntimeError("Could not build airfoil edges from generated profile points.")
